...
public class Board {
    ...
    Board(){
        for (int i = 0; i < 8; i++){
            this.board[0][i] = layout[i];
            this.board[1][i] = 1;
            this.board[6][i] = 1 + 8;
            this.board[7][i] = layout[i] + 8;
        }
    }
    ...
    int checkColour(int x, int y){
        if (this.checkOccupied(x, y)){
            if (this.getPiece(x, y) < 8){
                return 1;
            } else {
                return 2;
            }
    ...
    boolean checkPath1(int sX, int sY, int tX, int tY){
    ...
    public String cordsFKW(){return "" + this.findKingW()[0] + this.findKingW()[1];}

    public String cordsFKB(){return "" + this.findKingB()[0] + this.findKingB()[1];}

    boolean checkPathPart(int iY, int iX, int sY, int sX, int... block) {
        while (inBounds(iX, iY)) {
            for(int item : block) {
    ...
    boolean checkPathK(boolean white, int aX){
        ...
        if(!this.checkPathPart(cY + 1, cX, 1, 0, rook, queen))
            return false;

        if(!this.checkPathPart(cY - 1, cX, -1, 0, rook, queen))
            return false;

        if(!this.checkPathPart(cY, cX + 1, 0, 1, rook, queen))
            return false;

        if(!this.checkPathPart(cY, cX - 1, 0, -1, rook, queen))
            return false;

        if(!this.checkPathPart(cY + 1, cX + 1, 1, 1, bishop, queen))
            return false;
    ...
    boolean canCastle(int tX, boolean white){
        boolean canFinCastle;
        boolean canFinCastleK = (white ? this.CanCastle : this.CanCastleB);
        if (tX == 1){
            canFinCastle = (white ? this.CanCastleR0 : this.CanCastleR0B);
        } else {
            if (tX == 6){
                canFinCastle = (white ? this.CanCastleR7 : this.CanCastleR7B);
            }
        }
        boolean c = false;
        int x = tX - 3;
    ...
    boolean wantsMove2(int sX, int sY, int tX, int tY, boolean white){
    ...
    boolean isEnemy(int x, int y, boolean white){;
        ...
        boolean lower = this.board[y][x] < 9;
        boolean isEnemy = (white ? higher : lower);
        if (isEnemy) {
            return true;
        }
    ...
    boolean isAlly(int x, int y, boolean white){
        ...
        boolean isAlly = (white ? lower : higher);
        if (isAlly) {
            return true;
        }
        return false;
